/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the GridVOSslVpnResourceEntity type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GridVOSslVpnResourceEntity{}

// GridVOSslVpnResourceEntity struct for GridVOSslVpnResourceEntity
type GridVOSslVpnResourceEntity struct {
	// Current page number.
	CurrentPage *int32 `json:"currentPage,omitempty"`
	// Number of entries per page.
	CurrentSize *int32 `json:"currentSize,omitempty"`
	Data []SslVpnResourceEntity `json:"data,omitempty"`
	// Total rows of all items.
	TotalRows *int64 `json:"totalRows,omitempty"`
}

// NewGridVOSslVpnResourceEntity instantiates a new GridVOSslVpnResourceEntity object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGridVOSslVpnResourceEntity() *GridVOSslVpnResourceEntity {
	this := GridVOSslVpnResourceEntity{}
	return &this
}

// NewGridVOSslVpnResourceEntityWithDefaults instantiates a new GridVOSslVpnResourceEntity object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGridVOSslVpnResourceEntityWithDefaults() *GridVOSslVpnResourceEntity {
	this := GridVOSslVpnResourceEntity{}
	return &this
}

// GetCurrentPage returns the CurrentPage field value if set, zero value otherwise.
func (o *GridVOSslVpnResourceEntity) GetCurrentPage() int32 {
	if o == nil || IsNil(o.CurrentPage) {
		var ret int32
		return ret
	}
	return *o.CurrentPage
}

// GetCurrentPageOk returns a tuple with the CurrentPage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GridVOSslVpnResourceEntity) GetCurrentPageOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentPage) {
		return nil, false
	}
	return o.CurrentPage, true
}

// HasCurrentPage returns a boolean if a field has been set.
func (o *GridVOSslVpnResourceEntity) HasCurrentPage() bool {
	if o != nil && !IsNil(o.CurrentPage) {
		return true
	}

	return false
}

// SetCurrentPage gets a reference to the given int32 and assigns it to the CurrentPage field.
func (o *GridVOSslVpnResourceEntity) SetCurrentPage(v int32) {
	o.CurrentPage = &v
}

// GetCurrentSize returns the CurrentSize field value if set, zero value otherwise.
func (o *GridVOSslVpnResourceEntity) GetCurrentSize() int32 {
	if o == nil || IsNil(o.CurrentSize) {
		var ret int32
		return ret
	}
	return *o.CurrentSize
}

// GetCurrentSizeOk returns a tuple with the CurrentSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GridVOSslVpnResourceEntity) GetCurrentSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentSize) {
		return nil, false
	}
	return o.CurrentSize, true
}

// HasCurrentSize returns a boolean if a field has been set.
func (o *GridVOSslVpnResourceEntity) HasCurrentSize() bool {
	if o != nil && !IsNil(o.CurrentSize) {
		return true
	}

	return false
}

// SetCurrentSize gets a reference to the given int32 and assigns it to the CurrentSize field.
func (o *GridVOSslVpnResourceEntity) SetCurrentSize(v int32) {
	o.CurrentSize = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *GridVOSslVpnResourceEntity) GetData() []SslVpnResourceEntity {
	if o == nil || IsNil(o.Data) {
		var ret []SslVpnResourceEntity
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GridVOSslVpnResourceEntity) GetDataOk() ([]SslVpnResourceEntity, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *GridVOSslVpnResourceEntity) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []SslVpnResourceEntity and assigns it to the Data field.
func (o *GridVOSslVpnResourceEntity) SetData(v []SslVpnResourceEntity) {
	o.Data = v
}

// GetTotalRows returns the TotalRows field value if set, zero value otherwise.
func (o *GridVOSslVpnResourceEntity) GetTotalRows() int64 {
	if o == nil || IsNil(o.TotalRows) {
		var ret int64
		return ret
	}
	return *o.TotalRows
}

// GetTotalRowsOk returns a tuple with the TotalRows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GridVOSslVpnResourceEntity) GetTotalRowsOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalRows) {
		return nil, false
	}
	return o.TotalRows, true
}

// HasTotalRows returns a boolean if a field has been set.
func (o *GridVOSslVpnResourceEntity) HasTotalRows() bool {
	if o != nil && !IsNil(o.TotalRows) {
		return true
	}

	return false
}

// SetTotalRows gets a reference to the given int64 and assigns it to the TotalRows field.
func (o *GridVOSslVpnResourceEntity) SetTotalRows(v int64) {
	o.TotalRows = &v
}

func (o GridVOSslVpnResourceEntity) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GridVOSslVpnResourceEntity) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CurrentPage) {
		toSerialize["currentPage"] = o.CurrentPage
	}
	if !IsNil(o.CurrentSize) {
		toSerialize["currentSize"] = o.CurrentSize
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.TotalRows) {
		toSerialize["totalRows"] = o.TotalRows
	}
	return toSerialize, nil
}

type NullableGridVOSslVpnResourceEntity struct {
	value *GridVOSslVpnResourceEntity
	isSet bool
}

func (v NullableGridVOSslVpnResourceEntity) Get() *GridVOSslVpnResourceEntity {
	return v.value
}

func (v *NullableGridVOSslVpnResourceEntity) Set(val *GridVOSslVpnResourceEntity) {
	v.value = val
	v.isSet = true
}

func (v NullableGridVOSslVpnResourceEntity) IsSet() bool {
	return v.isSet
}

func (v *NullableGridVOSslVpnResourceEntity) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGridVOSslVpnResourceEntity(val *GridVOSslVpnResourceEntity) *NullableGridVOSslVpnResourceEntity {
	return &NullableGridVOSslVpnResourceEntity{value: val, isSet: true}
}

func (v NullableGridVOSslVpnResourceEntity) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGridVOSslVpnResourceEntity) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


