...
GetDadLinkPortOk returns a tuple with the DadLinkPort field if it's non-nil, zero value otherwise
...
GetPeerLinkPortOk returns a tuple with the PeerLinkPort field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.
