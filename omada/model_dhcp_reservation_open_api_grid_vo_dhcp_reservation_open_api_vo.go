/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the DhcpReservationOpenApiGridVODhcpReservationOpenApiVO type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DhcpReservationOpenApiGridVODhcpReservationOpenApiVO{}

// DhcpReservationOpenApiGridVODhcpReservationOpenApiVO struct for DhcpReservationOpenApiGridVODhcpReservationOpenApiVO
type DhcpReservationOpenApiGridVODhcpReservationOpenApiVO struct {
	// Current page number.
	CurrentPage *int32 `json:"currentPage,omitempty"`
	// Number of entries per page.
	CurrentSize *int32 `json:"currentSize,omitempty"`
	Data []DhcpReservationOpenApiVO `json:"data,omitempty"`
	// Mapping between lan network name and lan network ID
	NetNameToIdMap *map[string]string `json:"netNameToIdMap,omitempty"`
	// Options supported by DHCP Reservation.
	SupportOptions []int32 `json:"supportOptions,omitempty"`
	// Total rows of all items.
	TotalRows *int64 `json:"totalRows,omitempty"`
}

// NewDhcpReservationOpenApiGridVODhcpReservationOpenApiVO instantiates a new DhcpReservationOpenApiGridVODhcpReservationOpenApiVO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDhcpReservationOpenApiGridVODhcpReservationOpenApiVO() *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO {
	this := DhcpReservationOpenApiGridVODhcpReservationOpenApiVO{}
	return &this
}

// NewDhcpReservationOpenApiGridVODhcpReservationOpenApiVOWithDefaults instantiates a new DhcpReservationOpenApiGridVODhcpReservationOpenApiVO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDhcpReservationOpenApiGridVODhcpReservationOpenApiVOWithDefaults() *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO {
	this := DhcpReservationOpenApiGridVODhcpReservationOpenApiVO{}
	return &this
}

// GetCurrentPage returns the CurrentPage field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetCurrentPage() int32 {
	if o == nil || IsNil(o.CurrentPage) {
		var ret int32
		return ret
	}
	return *o.CurrentPage
}

// GetCurrentPageOk returns a tuple with the CurrentPage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetCurrentPageOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentPage) {
		return nil, false
	}
	return o.CurrentPage, true
}

// HasCurrentPage returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasCurrentPage() bool {
	if o != nil && !IsNil(o.CurrentPage) {
		return true
	}

	return false
}

// SetCurrentPage gets a reference to the given int32 and assigns it to the CurrentPage field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetCurrentPage(v int32) {
	o.CurrentPage = &v
}

// GetCurrentSize returns the CurrentSize field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetCurrentSize() int32 {
	if o == nil || IsNil(o.CurrentSize) {
		var ret int32
		return ret
	}
	return *o.CurrentSize
}

// GetCurrentSizeOk returns a tuple with the CurrentSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetCurrentSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentSize) {
		return nil, false
	}
	return o.CurrentSize, true
}

// HasCurrentSize returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasCurrentSize() bool {
	if o != nil && !IsNil(o.CurrentSize) {
		return true
	}

	return false
}

// SetCurrentSize gets a reference to the given int32 and assigns it to the CurrentSize field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetCurrentSize(v int32) {
	o.CurrentSize = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetData() []DhcpReservationOpenApiVO {
	if o == nil || IsNil(o.Data) {
		var ret []DhcpReservationOpenApiVO
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetDataOk() ([]DhcpReservationOpenApiVO, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []DhcpReservationOpenApiVO and assigns it to the Data field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetData(v []DhcpReservationOpenApiVO) {
	o.Data = v
}

// GetNetNameToIdMap returns the NetNameToIdMap field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetNetNameToIdMap() map[string]string {
	if o == nil || IsNil(o.NetNameToIdMap) {
		var ret map[string]string
		return ret
	}
	return *o.NetNameToIdMap
}

// GetNetNameToIdMapOk returns a tuple with the NetNameToIdMap field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetNetNameToIdMapOk() (*map[string]string, bool) {
	if o == nil || IsNil(o.NetNameToIdMap) {
		return nil, false
	}
	return o.NetNameToIdMap, true
}

// HasNetNameToIdMap returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasNetNameToIdMap() bool {
	if o != nil && !IsNil(o.NetNameToIdMap) {
		return true
	}

	return false
}

// SetNetNameToIdMap gets a reference to the given map[string]string and assigns it to the NetNameToIdMap field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetNetNameToIdMap(v map[string]string) {
	o.NetNameToIdMap = &v
}

// GetSupportOptions returns the SupportOptions field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetSupportOptions() []int32 {
	if o == nil || IsNil(o.SupportOptions) {
		var ret []int32
		return ret
	}
	return o.SupportOptions
}

// GetSupportOptionsOk returns a tuple with the SupportOptions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetSupportOptionsOk() ([]int32, bool) {
	if o == nil || IsNil(o.SupportOptions) {
		return nil, false
	}
	return o.SupportOptions, true
}

// HasSupportOptions returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasSupportOptions() bool {
	if o != nil && !IsNil(o.SupportOptions) {
		return true
	}

	return false
}

// SetSupportOptions gets a reference to the given []int32 and assigns it to the SupportOptions field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetSupportOptions(v []int32) {
	o.SupportOptions = v
}

// GetTotalRows returns the TotalRows field value if set, zero value otherwise.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetTotalRows() int64 {
	if o == nil || IsNil(o.TotalRows) {
		var ret int64
		return ret
	}
	return *o.TotalRows
}

// GetTotalRowsOk returns a tuple with the TotalRows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) GetTotalRowsOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalRows) {
		return nil, false
	}
	return o.TotalRows, true
}

// HasTotalRows returns a boolean if a field has been set.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) HasTotalRows() bool {
	if o != nil && !IsNil(o.TotalRows) {
		return true
	}

	return false
}

// SetTotalRows gets a reference to the given int64 and assigns it to the TotalRows field.
func (o *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) SetTotalRows(v int64) {
	o.TotalRows = &v
}

func (o DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CurrentPage) {
		toSerialize["currentPage"] = o.CurrentPage
	}
	if !IsNil(o.CurrentSize) {
		toSerialize["currentSize"] = o.CurrentSize
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.NetNameToIdMap) {
		toSerialize["netNameToIdMap"] = o.NetNameToIdMap
	}
	if !IsNil(o.SupportOptions) {
		toSerialize["supportOptions"] = o.SupportOptions
	}
	if !IsNil(o.TotalRows) {
		toSerialize["totalRows"] = o.TotalRows
	}
	return toSerialize, nil
}

type NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO struct {
	value *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO
	isSet bool
}

func (v NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) Get() *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO {
	return v.value
}

func (v *NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) Set(val *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) {
	v.value = val
	v.isSet = true
}

func (v NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) IsSet() bool {
	return v.isSet
}

func (v *NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO(val *DhcpReservationOpenApiGridVODhcpReservationOpenApiVO) *NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO {
	return &NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO{value: val, isSet: true}
}

func (v NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDhcpReservationOpenApiGridVODhcpReservationOpenApiVO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


