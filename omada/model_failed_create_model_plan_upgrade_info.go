/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the FailedCreateModelPlanUpgradeInfo type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FailedCreateModelPlanUpgradeInfo{}

// FailedCreateModelPlanUpgradeInfo struct for FailedCreateModelPlanUpgradeInfo
type FailedCreateModelPlanUpgradeInfo struct {
	// The day of the month selected by the user, It should be within the range of 1-31
	DayOfMonth *int32 `json:"dayOfMonth,omitempty"`
	// The hour of the day selected by the user, It should be within the range of 0-23
	Hour *int32 `json:"hour,omitempty"`
	// The minute in the hour selected by the user, It should be within the range of 0-59
	Minute *int32 `json:"minute,omitempty"`
	// The month of the year selected by the user, It should be within the range of 1-12
	MonthOfYear *int32 `json:"monthOfYear,omitempty"`
	// The type of upgrade execution time, where 0 represents now and 1 represents the specified time
	ScheduleType int32 `json:"scheduleType"`
	// The year selected by the user
	Year *int32 `json:"year,omitempty"`
}

type _FailedCreateModelPlanUpgradeInfo FailedCreateModelPlanUpgradeInfo

// NewFailedCreateModelPlanUpgradeInfo instantiates a new FailedCreateModelPlanUpgradeInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFailedCreateModelPlanUpgradeInfo(scheduleType int32) *FailedCreateModelPlanUpgradeInfo {
	this := FailedCreateModelPlanUpgradeInfo{}
	this.ScheduleType = scheduleType
	return &this
}

// NewFailedCreateModelPlanUpgradeInfoWithDefaults instantiates a new FailedCreateModelPlanUpgradeInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFailedCreateModelPlanUpgradeInfoWithDefaults() *FailedCreateModelPlanUpgradeInfo {
	this := FailedCreateModelPlanUpgradeInfo{}
	return &this
}

// GetDayOfMonth returns the DayOfMonth field value if set, zero value otherwise.
func (o *FailedCreateModelPlanUpgradeInfo) GetDayOfMonth() int32 {
	if o == nil || IsNil(o.DayOfMonth) {
		var ret int32
		return ret
	}
	return *o.DayOfMonth
}

// GetDayOfMonthOk returns a tuple with the DayOfMonth field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetDayOfMonthOk() (*int32, bool) {
	if o == nil || IsNil(o.DayOfMonth) {
		return nil, false
	}
	return o.DayOfMonth, true
}

// HasDayOfMonth returns a boolean if a field has been set.
func (o *FailedCreateModelPlanUpgradeInfo) HasDayOfMonth() bool {
	if o != nil && !IsNil(o.DayOfMonth) {
		return true
	}

	return false
}

// SetDayOfMonth gets a reference to the given int32 and assigns it to the DayOfMonth field.
func (o *FailedCreateModelPlanUpgradeInfo) SetDayOfMonth(v int32) {
	o.DayOfMonth = &v
}

// GetHour returns the Hour field value if set, zero value otherwise.
func (o *FailedCreateModelPlanUpgradeInfo) GetHour() int32 {
	if o == nil || IsNil(o.Hour) {
		var ret int32
		return ret
	}
	return *o.Hour
}

// GetHourOk returns a tuple with the Hour field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetHourOk() (*int32, bool) {
	if o == nil || IsNil(o.Hour) {
		return nil, false
	}
	return o.Hour, true
}

// HasHour returns a boolean if a field has been set.
func (o *FailedCreateModelPlanUpgradeInfo) HasHour() bool {
	if o != nil && !IsNil(o.Hour) {
		return true
	}

	return false
}

// SetHour gets a reference to the given int32 and assigns it to the Hour field.
func (o *FailedCreateModelPlanUpgradeInfo) SetHour(v int32) {
	o.Hour = &v
}

// GetMinute returns the Minute field value if set, zero value otherwise.
func (o *FailedCreateModelPlanUpgradeInfo) GetMinute() int32 {
	if o == nil || IsNil(o.Minute) {
		var ret int32
		return ret
	}
	return *o.Minute
}

// GetMinuteOk returns a tuple with the Minute field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetMinuteOk() (*int32, bool) {
	if o == nil || IsNil(o.Minute) {
		return nil, false
	}
	return o.Minute, true
}

// HasMinute returns a boolean if a field has been set.
func (o *FailedCreateModelPlanUpgradeInfo) HasMinute() bool {
	if o != nil && !IsNil(o.Minute) {
		return true
	}

	return false
}

// SetMinute gets a reference to the given int32 and assigns it to the Minute field.
func (o *FailedCreateModelPlanUpgradeInfo) SetMinute(v int32) {
	o.Minute = &v
}

// GetMonthOfYear returns the MonthOfYear field value if set, zero value otherwise.
func (o *FailedCreateModelPlanUpgradeInfo) GetMonthOfYear() int32 {
	if o == nil || IsNil(o.MonthOfYear) {
		var ret int32
		return ret
	}
	return *o.MonthOfYear
}

// GetMonthOfYearOk returns a tuple with the MonthOfYear field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetMonthOfYearOk() (*int32, bool) {
	if o == nil || IsNil(o.MonthOfYear) {
		return nil, false
	}
	return o.MonthOfYear, true
}

// HasMonthOfYear returns a boolean if a field has been set.
func (o *FailedCreateModelPlanUpgradeInfo) HasMonthOfYear() bool {
	if o != nil && !IsNil(o.MonthOfYear) {
		return true
	}

	return false
}

// SetMonthOfYear gets a reference to the given int32 and assigns it to the MonthOfYear field.
func (o *FailedCreateModelPlanUpgradeInfo) SetMonthOfYear(v int32) {
	o.MonthOfYear = &v
}

// GetScheduleType returns the ScheduleType field value
func (o *FailedCreateModelPlanUpgradeInfo) GetScheduleType() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.ScheduleType
}

// GetScheduleTypeOk returns a tuple with the ScheduleType field value
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetScheduleTypeOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ScheduleType, true
}

// SetScheduleType sets field value
func (o *FailedCreateModelPlanUpgradeInfo) SetScheduleType(v int32) {
	o.ScheduleType = v
}

// GetYear returns the Year field value if set, zero value otherwise.
func (o *FailedCreateModelPlanUpgradeInfo) GetYear() int32 {
	if o == nil || IsNil(o.Year) {
		var ret int32
		return ret
	}
	return *o.Year
}

// GetYearOk returns a tuple with the Year field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FailedCreateModelPlanUpgradeInfo) GetYearOk() (*int32, bool) {
	if o == nil || IsNil(o.Year) {
		return nil, false
	}
	return o.Year, true
}

// HasYear returns a boolean if a field has been set.
func (o *FailedCreateModelPlanUpgradeInfo) HasYear() bool {
	if o != nil && !IsNil(o.Year) {
		return true
	}

	return false
}

// SetYear gets a reference to the given int32 and assigns it to the Year field.
func (o *FailedCreateModelPlanUpgradeInfo) SetYear(v int32) {
	o.Year = &v
}

func (o FailedCreateModelPlanUpgradeInfo) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FailedCreateModelPlanUpgradeInfo) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.DayOfMonth) {
		toSerialize["dayOfMonth"] = o.DayOfMonth
	}
	if !IsNil(o.Hour) {
		toSerialize["hour"] = o.Hour
	}
	if !IsNil(o.Minute) {
		toSerialize["minute"] = o.Minute
	}
	if !IsNil(o.MonthOfYear) {
		toSerialize["monthOfYear"] = o.MonthOfYear
	}
	toSerialize["scheduleType"] = o.ScheduleType
	if !IsNil(o.Year) {
		toSerialize["year"] = o.Year
	}
	return toSerialize, nil
}

func (o *FailedCreateModelPlanUpgradeInfo) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"scheduleType",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFailedCreateModelPlanUpgradeInfo := _FailedCreateModelPlanUpgradeInfo{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varFailedCreateModelPlanUpgradeInfo)

	if err != nil {
		return err
	}

	*o = FailedCreateModelPlanUpgradeInfo(varFailedCreateModelPlanUpgradeInfo)

	return err
}

type NullableFailedCreateModelPlanUpgradeInfo struct {
	value *FailedCreateModelPlanUpgradeInfo
	isSet bool
}

func (v NullableFailedCreateModelPlanUpgradeInfo) Get() *FailedCreateModelPlanUpgradeInfo {
	return v.value
}

func (v *NullableFailedCreateModelPlanUpgradeInfo) Set(val *FailedCreateModelPlanUpgradeInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableFailedCreateModelPlanUpgradeInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableFailedCreateModelPlanUpgradeInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFailedCreateModelPlanUpgradeInfo(val *FailedCreateModelPlanUpgradeInfo) *NullableFailedCreateModelPlanUpgradeInfo {
	return &NullableFailedCreateModelPlanUpgradeInfo{value: val, isSet: true}
}

func (v NullableFailedCreateModelPlanUpgradeInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFailedCreateModelPlanUpgradeInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


