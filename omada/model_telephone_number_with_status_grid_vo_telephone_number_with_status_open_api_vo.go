/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO{}

// TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO struct for TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO
type TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO struct {
	// Current page number.
	CurrentPage *int32 `json:"currentPage,omitempty"`
	// Number of entries per page.
	CurrentSize *int32 `json:"currentSize,omitempty"`
	Data []TelephoneNumberWithStatusOpenApiVO `json:"data,omitempty"`
	// Total rows of all items.
	TotalRows *int64 `json:"totalRows,omitempty"`
	VoipPhoneNumberPasswordLimit *int32 `json:"voipPhoneNumberPasswordLimit,omitempty"`
}

// NewTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO instantiates a new TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO() *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO {
	this := TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO{}
	return &this
}

// NewTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVOWithDefaults instantiates a new TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVOWithDefaults() *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO {
	this := TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO{}
	return &this
}

// GetCurrentPage returns the CurrentPage field value if set, zero value otherwise.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetCurrentPage() int32 {
	if o == nil || IsNil(o.CurrentPage) {
		var ret int32
		return ret
	}
	return *o.CurrentPage
}

// GetCurrentPageOk returns a tuple with the CurrentPage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetCurrentPageOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentPage) {
		return nil, false
	}
	return o.CurrentPage, true
}

// HasCurrentPage returns a boolean if a field has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) HasCurrentPage() bool {
	if o != nil && !IsNil(o.CurrentPage) {
		return true
	}

	return false
}

// SetCurrentPage gets a reference to the given int32 and assigns it to the CurrentPage field.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) SetCurrentPage(v int32) {
	o.CurrentPage = &v
}

// GetCurrentSize returns the CurrentSize field value if set, zero value otherwise.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetCurrentSize() int32 {
	if o == nil || IsNil(o.CurrentSize) {
		var ret int32
		return ret
	}
	return *o.CurrentSize
}

// GetCurrentSizeOk returns a tuple with the CurrentSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetCurrentSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentSize) {
		return nil, false
	}
	return o.CurrentSize, true
}

// HasCurrentSize returns a boolean if a field has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) HasCurrentSize() bool {
	if o != nil && !IsNil(o.CurrentSize) {
		return true
	}

	return false
}

// SetCurrentSize gets a reference to the given int32 and assigns it to the CurrentSize field.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) SetCurrentSize(v int32) {
	o.CurrentSize = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetData() []TelephoneNumberWithStatusOpenApiVO {
	if o == nil || IsNil(o.Data) {
		var ret []TelephoneNumberWithStatusOpenApiVO
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetDataOk() ([]TelephoneNumberWithStatusOpenApiVO, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []TelephoneNumberWithStatusOpenApiVO and assigns it to the Data field.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) SetData(v []TelephoneNumberWithStatusOpenApiVO) {
	o.Data = v
}

// GetTotalRows returns the TotalRows field value if set, zero value otherwise.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetTotalRows() int64 {
	if o == nil || IsNil(o.TotalRows) {
		var ret int64
		return ret
	}
	return *o.TotalRows
}

// GetTotalRowsOk returns a tuple with the TotalRows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetTotalRowsOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalRows) {
		return nil, false
	}
	return o.TotalRows, true
}

// HasTotalRows returns a boolean if a field has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) HasTotalRows() bool {
	if o != nil && !IsNil(o.TotalRows) {
		return true
	}

	return false
}

// SetTotalRows gets a reference to the given int64 and assigns it to the TotalRows field.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) SetTotalRows(v int64) {
	o.TotalRows = &v
}

// GetVoipPhoneNumberPasswordLimit returns the VoipPhoneNumberPasswordLimit field value if set, zero value otherwise.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetVoipPhoneNumberPasswordLimit() int32 {
	if o == nil || IsNil(o.VoipPhoneNumberPasswordLimit) {
		var ret int32
		return ret
	}
	return *o.VoipPhoneNumberPasswordLimit
}

// GetVoipPhoneNumberPasswordLimitOk returns a tuple with the VoipPhoneNumberPasswordLimit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) GetVoipPhoneNumberPasswordLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.VoipPhoneNumberPasswordLimit) {
		return nil, false
	}
	return o.VoipPhoneNumberPasswordLimit, true
}

// HasVoipPhoneNumberPasswordLimit returns a boolean if a field has been set.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) HasVoipPhoneNumberPasswordLimit() bool {
	if o != nil && !IsNil(o.VoipPhoneNumberPasswordLimit) {
		return true
	}

	return false
}

// SetVoipPhoneNumberPasswordLimit gets a reference to the given int32 and assigns it to the VoipPhoneNumberPasswordLimit field.
func (o *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) SetVoipPhoneNumberPasswordLimit(v int32) {
	o.VoipPhoneNumberPasswordLimit = &v
}

func (o TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CurrentPage) {
		toSerialize["currentPage"] = o.CurrentPage
	}
	if !IsNil(o.CurrentSize) {
		toSerialize["currentSize"] = o.CurrentSize
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.TotalRows) {
		toSerialize["totalRows"] = o.TotalRows
	}
	if !IsNil(o.VoipPhoneNumberPasswordLimit) {
		toSerialize["voipPhoneNumberPasswordLimit"] = o.VoipPhoneNumberPasswordLimit
	}
	return toSerialize, nil
}

type NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO struct {
	value *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO
	isSet bool
}

func (v NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) Get() *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO {
	return v.value
}

func (v *NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) Set(val *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) {
	v.value = val
	v.isSet = true
}

func (v NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) IsSet() bool {
	return v.isSet
}

func (v *NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO(val *TelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) *NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO {
	return &NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO{value: val, isSet: true}
}

func (v NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTelephoneNumberWithStatusGridVOTelephoneNumberWithStatusOpenApiVO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


