/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the ClientStatisticalDataDetailResult type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ClientStatisticalDataDetailResult{}

// ClientStatisticalDataDetailResult struct for ClientStatisticalDataDetailResult
type ClientStatisticalDataDetailResult struct {
	// Average downlink rate (Byte/s).
	AvgDownRate *int64 `json:"avgDownRate,omitempty"`
	// (Wireless) Average uplink negotiation rate (bit/s).
	AvgRxR *int64 `json:"avgRxR,omitempty"`
	// (Wireless) Average signal strength, unit: dBm.
	AvgSignal *int32 `json:"avgSignal,omitempty"`
	// (Wireless) Average downlink negotiation rate (bit/s).
	AvgTxR *int64 `json:"avgTxR,omitempty"`
	// Average uplink rate (Byte/s).
	AvgUpRate *int64 `json:"avgUpRate,omitempty"`
	// Client Statistical Data Detail list.
	Stats []ClientStatisticalDataDetail `json:"stats,omitempty"`
	// Client total downstream traffic (Byte).
	TotalDown *int64 `json:"totalDown,omitempty"`
	// Total number of downstream failed packets.
	TotalTxFP *int64 `json:"totalTxFP,omitempty"`
	// Client total upstream traffic (Byte).
	TotalUp *int64 `json:"totalUp,omitempty"`
}

// NewClientStatisticalDataDetailResult instantiates a new ClientStatisticalDataDetailResult object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewClientStatisticalDataDetailResult() *ClientStatisticalDataDetailResult {
	this := ClientStatisticalDataDetailResult{}
	return &this
}

// NewClientStatisticalDataDetailResultWithDefaults instantiates a new ClientStatisticalDataDetailResult object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewClientStatisticalDataDetailResultWithDefaults() *ClientStatisticalDataDetailResult {
	this := ClientStatisticalDataDetailResult{}
	return &this
}

// GetAvgDownRate returns the AvgDownRate field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetAvgDownRate() int64 {
	if o == nil || IsNil(o.AvgDownRate) {
		var ret int64
		return ret
	}
	return *o.AvgDownRate
}

// GetAvgDownRateOk returns a tuple with the AvgDownRate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetAvgDownRateOk() (*int64, bool) {
	if o == nil || IsNil(o.AvgDownRate) {
		return nil, false
	}
	return o.AvgDownRate, true
}

// HasAvgDownRate returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasAvgDownRate() bool {
	if o != nil && !IsNil(o.AvgDownRate) {
		return true
	}

	return false
}

// SetAvgDownRate gets a reference to the given int64 and assigns it to the AvgDownRate field.
func (o *ClientStatisticalDataDetailResult) SetAvgDownRate(v int64) {
	o.AvgDownRate = &v
}

// GetAvgRxR returns the AvgRxR field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetAvgRxR() int64 {
	if o == nil || IsNil(o.AvgRxR) {
		var ret int64
		return ret
	}
	return *o.AvgRxR
}

// GetAvgRxROk returns a tuple with the AvgRxR field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetAvgRxROk() (*int64, bool) {
	if o == nil || IsNil(o.AvgRxR) {
		return nil, false
	}
	return o.AvgRxR, true
}

// HasAvgRxR returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasAvgRxR() bool {
	if o != nil && !IsNil(o.AvgRxR) {
		return true
	}

	return false
}

// SetAvgRxR gets a reference to the given int64 and assigns it to the AvgRxR field.
func (o *ClientStatisticalDataDetailResult) SetAvgRxR(v int64) {
	o.AvgRxR = &v
}

// GetAvgSignal returns the AvgSignal field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetAvgSignal() int32 {
	if o == nil || IsNil(o.AvgSignal) {
		var ret int32
		return ret
	}
	return *o.AvgSignal
}

// GetAvgSignalOk returns a tuple with the AvgSignal field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetAvgSignalOk() (*int32, bool) {
	if o == nil || IsNil(o.AvgSignal) {
		return nil, false
	}
	return o.AvgSignal, true
}

// HasAvgSignal returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasAvgSignal() bool {
	if o != nil && !IsNil(o.AvgSignal) {
		return true
	}

	return false
}

// SetAvgSignal gets a reference to the given int32 and assigns it to the AvgSignal field.
func (o *ClientStatisticalDataDetailResult) SetAvgSignal(v int32) {
	o.AvgSignal = &v
}

// GetAvgTxR returns the AvgTxR field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetAvgTxR() int64 {
	if o == nil || IsNil(o.AvgTxR) {
		var ret int64
		return ret
	}
	return *o.AvgTxR
}

// GetAvgTxROk returns a tuple with the AvgTxR field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetAvgTxROk() (*int64, bool) {
	if o == nil || IsNil(o.AvgTxR) {
		return nil, false
	}
	return o.AvgTxR, true
}

// HasAvgTxR returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasAvgTxR() bool {
	if o != nil && !IsNil(o.AvgTxR) {
		return true
	}

	return false
}

// SetAvgTxR gets a reference to the given int64 and assigns it to the AvgTxR field.
func (o *ClientStatisticalDataDetailResult) SetAvgTxR(v int64) {
	o.AvgTxR = &v
}

// GetAvgUpRate returns the AvgUpRate field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetAvgUpRate() int64 {
	if o == nil || IsNil(o.AvgUpRate) {
		var ret int64
		return ret
	}
	return *o.AvgUpRate
}

// GetAvgUpRateOk returns a tuple with the AvgUpRate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetAvgUpRateOk() (*int64, bool) {
	if o == nil || IsNil(o.AvgUpRate) {
		return nil, false
	}
	return o.AvgUpRate, true
}

// HasAvgUpRate returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasAvgUpRate() bool {
	if o != nil && !IsNil(o.AvgUpRate) {
		return true
	}

	return false
}

// SetAvgUpRate gets a reference to the given int64 and assigns it to the AvgUpRate field.
func (o *ClientStatisticalDataDetailResult) SetAvgUpRate(v int64) {
	o.AvgUpRate = &v
}

// GetStats returns the Stats field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetStats() []ClientStatisticalDataDetail {
	if o == nil || IsNil(o.Stats) {
		var ret []ClientStatisticalDataDetail
		return ret
	}
	return o.Stats
}

// GetStatsOk returns a tuple with the Stats field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetStatsOk() ([]ClientStatisticalDataDetail, bool) {
	if o == nil || IsNil(o.Stats) {
		return nil, false
	}
	return o.Stats, true
}

// HasStats returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasStats() bool {
	if o != nil && !IsNil(o.Stats) {
		return true
	}

	return false
}

// SetStats gets a reference to the given []ClientStatisticalDataDetail and assigns it to the Stats field.
func (o *ClientStatisticalDataDetailResult) SetStats(v []ClientStatisticalDataDetail) {
	o.Stats = v
}

// GetTotalDown returns the TotalDown field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetTotalDown() int64 {
	if o == nil || IsNil(o.TotalDown) {
		var ret int64
		return ret
	}
	return *o.TotalDown
}

// GetTotalDownOk returns a tuple with the TotalDown field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetTotalDownOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalDown) {
		return nil, false
	}
	return o.TotalDown, true
}

// HasTotalDown returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasTotalDown() bool {
	if o != nil && !IsNil(o.TotalDown) {
		return true
	}

	return false
}

// SetTotalDown gets a reference to the given int64 and assigns it to the TotalDown field.
func (o *ClientStatisticalDataDetailResult) SetTotalDown(v int64) {
	o.TotalDown = &v
}

// GetTotalTxFP returns the TotalTxFP field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetTotalTxFP() int64 {
	if o == nil || IsNil(o.TotalTxFP) {
		var ret int64
		return ret
	}
	return *o.TotalTxFP
}

// GetTotalTxFPOk returns a tuple with the TotalTxFP field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetTotalTxFPOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalTxFP) {
		return nil, false
	}
	return o.TotalTxFP, true
}

// HasTotalTxFP returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasTotalTxFP() bool {
	if o != nil && !IsNil(o.TotalTxFP) {
		return true
	}

	return false
}

// SetTotalTxFP gets a reference to the given int64 and assigns it to the TotalTxFP field.
func (o *ClientStatisticalDataDetailResult) SetTotalTxFP(v int64) {
	o.TotalTxFP = &v
}

// GetTotalUp returns the TotalUp field value if set, zero value otherwise.
func (o *ClientStatisticalDataDetailResult) GetTotalUp() int64 {
	if o == nil || IsNil(o.TotalUp) {
		var ret int64
		return ret
	}
	return *o.TotalUp
}

// GetTotalUpOk returns a tuple with the TotalUp field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientStatisticalDataDetailResult) GetTotalUpOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalUp) {
		return nil, false
	}
	return o.TotalUp, true
}

// HasTotalUp returns a boolean if a field has been set.
func (o *ClientStatisticalDataDetailResult) HasTotalUp() bool {
	if o != nil && !IsNil(o.TotalUp) {
		return true
	}

	return false
}

// SetTotalUp gets a reference to the given int64 and assigns it to the TotalUp field.
func (o *ClientStatisticalDataDetailResult) SetTotalUp(v int64) {
	o.TotalUp = &v
}

func (o ClientStatisticalDataDetailResult) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ClientStatisticalDataDetailResult) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AvgDownRate) {
		toSerialize["avgDownRate"] = o.AvgDownRate
	}
	if !IsNil(o.AvgRxR) {
		toSerialize["avgRxR"] = o.AvgRxR
	}
	if !IsNil(o.AvgSignal) {
		toSerialize["avgSignal"] = o.AvgSignal
	}
	if !IsNil(o.AvgTxR) {
		toSerialize["avgTxR"] = o.AvgTxR
	}
	if !IsNil(o.AvgUpRate) {
		toSerialize["avgUpRate"] = o.AvgUpRate
	}
	if !IsNil(o.Stats) {
		toSerialize["stats"] = o.Stats
	}
	if !IsNil(o.TotalDown) {
		toSerialize["totalDown"] = o.TotalDown
	}
	if !IsNil(o.TotalTxFP) {
		toSerialize["totalTxFP"] = o.TotalTxFP
	}
	if !IsNil(o.TotalUp) {
		toSerialize["totalUp"] = o.TotalUp
	}
	return toSerialize, nil
}

type NullableClientStatisticalDataDetailResult struct {
	value *ClientStatisticalDataDetailResult
	isSet bool
}

func (v NullableClientStatisticalDataDetailResult) Get() *ClientStatisticalDataDetailResult {
	return v.value
}

func (v *NullableClientStatisticalDataDetailResult) Set(val *ClientStatisticalDataDetailResult) {
	v.value = val
	v.isSet = true
}

func (v NullableClientStatisticalDataDetailResult) IsSet() bool {
	return v.isSet
}

func (v *NullableClientStatisticalDataDetailResult) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableClientStatisticalDataDetailResult(val *ClientStatisticalDataDetailResult) *NullableClientStatisticalDataDetailResult {
	return &NullableClientStatisticalDataDetailResult{value: val, isSet: true}
}

func (v NullableClientStatisticalDataDetailResult) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableClientStatisticalDataDetailResult) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


