/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the ClientConnectionInfo type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ClientConnectionInfo{}

// ClientConnectionInfo Client connection histories
type ClientConnectionInfo struct {
	// Duration, unit: ms
	Duration *int64 `json:"duration,omitempty"`
	// End timestamp, unit: ms
	End *int64 `json:"end,omitempty"`
	// Start timestamp, unit: ms
	Start *int64 `json:"start,omitempty"`
}

// NewClientConnectionInfo instantiates a new ClientConnectionInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewClientConnectionInfo() *ClientConnectionInfo {
	this := ClientConnectionInfo{}
	return &this
}

// NewClientConnectionInfoWithDefaults instantiates a new ClientConnectionInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewClientConnectionInfoWithDefaults() *ClientConnectionInfo {
	this := ClientConnectionInfo{}
	return &this
}

// GetDuration returns the Duration field value if set, zero value otherwise.
func (o *ClientConnectionInfo) GetDuration() int64 {
	if o == nil || IsNil(o.Duration) {
		var ret int64
		return ret
	}
	return *o.Duration
}

// GetDurationOk returns a tuple with the Duration field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientConnectionInfo) GetDurationOk() (*int64, bool) {
	if o == nil || IsNil(o.Duration) {
		return nil, false
	}
	return o.Duration, true
}

// HasDuration returns a boolean if a field has been set.
func (o *ClientConnectionInfo) HasDuration() bool {
	if o != nil && !IsNil(o.Duration) {
		return true
	}

	return false
}

// SetDuration gets a reference to the given int64 and assigns it to the Duration field.
func (o *ClientConnectionInfo) SetDuration(v int64) {
	o.Duration = &v
}

// GetEnd returns the End field value if set, zero value otherwise.
func (o *ClientConnectionInfo) GetEnd() int64 {
	if o == nil || IsNil(o.End) {
		var ret int64
		return ret
	}
	return *o.End
}

// GetEndOk returns a tuple with the End field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientConnectionInfo) GetEndOk() (*int64, bool) {
	if o == nil || IsNil(o.End) {
		return nil, false
	}
	return o.End, true
}

// HasEnd returns a boolean if a field has been set.
func (o *ClientConnectionInfo) HasEnd() bool {
	if o != nil && !IsNil(o.End) {
		return true
	}

	return false
}

// SetEnd gets a reference to the given int64 and assigns it to the End field.
func (o *ClientConnectionInfo) SetEnd(v int64) {
	o.End = &v
}

// GetStart returns the Start field value if set, zero value otherwise.
func (o *ClientConnectionInfo) GetStart() int64 {
	if o == nil || IsNil(o.Start) {
		var ret int64
		return ret
	}
	return *o.Start
}

// GetStartOk returns a tuple with the Start field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientConnectionInfo) GetStartOk() (*int64, bool) {
	if o == nil || IsNil(o.Start) {
		return nil, false
	}
	return o.Start, true
}

// HasStart returns a boolean if a field has been set.
func (o *ClientConnectionInfo) HasStart() bool {
	if o != nil && !IsNil(o.Start) {
		return true
	}

	return false
}

// SetStart gets a reference to the given int64 and assigns it to the Start field.
func (o *ClientConnectionInfo) SetStart(v int64) {
	o.Start = &v
}

func (o ClientConnectionInfo) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ClientConnectionInfo) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Duration) {
		toSerialize["duration"] = o.Duration
	}
	if !IsNil(o.End) {
		toSerialize["end"] = o.End
	}
	if !IsNil(o.Start) {
		toSerialize["start"] = o.Start
	}
	return toSerialize, nil
}

type NullableClientConnectionInfo struct {
	value *ClientConnectionInfo
	isSet bool
}

func (v NullableClientConnectionInfo) Get() *ClientConnectionInfo {
	return v.value
}

func (v *NullableClientConnectionInfo) Set(val *ClientConnectionInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableClientConnectionInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableClientConnectionInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableClientConnectionInfo(val *ClientConnectionInfo) *NullableClientConnectionInfo {
	return &NullableClientConnectionInfo{value: val, isSet: true}
}

func (v NullableClientConnectionInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableClientConnectionInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


