/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the PageResponsePonPortInformationDTO type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PageResponsePonPortInformationDTO{}

// PageResponsePonPortInformationDTO struct for PageResponsePonPortInformationDTO
type PageResponsePonPortInformationDTO struct {
	// Content
	Content []PonPortInformationDTO `json:"content,omitempty"`
	// Whether is in the first page
	First *bool `json:"first,omitempty"`
	// Whether is in the last page
	Last *bool `json:"last,omitempty"`
	// Number of the current page,starting from 0.
	Number *int32 `json:"number,omitempty"`
	// Actual number of current page
	NumberOfElements *int32 `json:"numberOfElements,omitempty"`
	// Page size
	Size *int32 `json:"size,omitempty"`
	// Total elements
	TotalElements *int64 `json:"totalElements,omitempty"`
	// Total page number
	TotalPages *int32 `json:"totalPages,omitempty"`
}

// NewPageResponsePonPortInformationDTO instantiates a new PageResponsePonPortInformationDTO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPageResponsePonPortInformationDTO() *PageResponsePonPortInformationDTO {
	this := PageResponsePonPortInformationDTO{}
	return &this
}

// NewPageResponsePonPortInformationDTOWithDefaults instantiates a new PageResponsePonPortInformationDTO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPageResponsePonPortInformationDTOWithDefaults() *PageResponsePonPortInformationDTO {
	this := PageResponsePonPortInformationDTO{}
	return &this
}

// GetContent returns the Content field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetContent() []PonPortInformationDTO {
	if o == nil || IsNil(o.Content) {
		var ret []PonPortInformationDTO
		return ret
	}
	return o.Content
}

// GetContentOk returns a tuple with the Content field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetContentOk() ([]PonPortInformationDTO, bool) {
	if o == nil || IsNil(o.Content) {
		return nil, false
	}
	return o.Content, true
}

// HasContent returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasContent() bool {
	if o != nil && !IsNil(o.Content) {
		return true
	}

	return false
}

// SetContent gets a reference to the given []PonPortInformationDTO and assigns it to the Content field.
func (o *PageResponsePonPortInformationDTO) SetContent(v []PonPortInformationDTO) {
	o.Content = v
}

// GetFirst returns the First field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetFirst() bool {
	if o == nil || IsNil(o.First) {
		var ret bool
		return ret
	}
	return *o.First
}

// GetFirstOk returns a tuple with the First field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetFirstOk() (*bool, bool) {
	if o == nil || IsNil(o.First) {
		return nil, false
	}
	return o.First, true
}

// HasFirst returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasFirst() bool {
	if o != nil && !IsNil(o.First) {
		return true
	}

	return false
}

// SetFirst gets a reference to the given bool and assigns it to the First field.
func (o *PageResponsePonPortInformationDTO) SetFirst(v bool) {
	o.First = &v
}

// GetLast returns the Last field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetLast() bool {
	if o == nil || IsNil(o.Last) {
		var ret bool
		return ret
	}
	return *o.Last
}

// GetLastOk returns a tuple with the Last field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetLastOk() (*bool, bool) {
	if o == nil || IsNil(o.Last) {
		return nil, false
	}
	return o.Last, true
}

// HasLast returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasLast() bool {
	if o != nil && !IsNil(o.Last) {
		return true
	}

	return false
}

// SetLast gets a reference to the given bool and assigns it to the Last field.
func (o *PageResponsePonPortInformationDTO) SetLast(v bool) {
	o.Last = &v
}

// GetNumber returns the Number field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetNumber() int32 {
	if o == nil || IsNil(o.Number) {
		var ret int32
		return ret
	}
	return *o.Number
}

// GetNumberOk returns a tuple with the Number field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetNumberOk() (*int32, bool) {
	if o == nil || IsNil(o.Number) {
		return nil, false
	}
	return o.Number, true
}

// HasNumber returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasNumber() bool {
	if o != nil && !IsNil(o.Number) {
		return true
	}

	return false
}

// SetNumber gets a reference to the given int32 and assigns it to the Number field.
func (o *PageResponsePonPortInformationDTO) SetNumber(v int32) {
	o.Number = &v
}

// GetNumberOfElements returns the NumberOfElements field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetNumberOfElements() int32 {
	if o == nil || IsNil(o.NumberOfElements) {
		var ret int32
		return ret
	}
	return *o.NumberOfElements
}

// GetNumberOfElementsOk returns a tuple with the NumberOfElements field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetNumberOfElementsOk() (*int32, bool) {
	if o == nil || IsNil(o.NumberOfElements) {
		return nil, false
	}
	return o.NumberOfElements, true
}

// HasNumberOfElements returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasNumberOfElements() bool {
	if o != nil && !IsNil(o.NumberOfElements) {
		return true
	}

	return false
}

// SetNumberOfElements gets a reference to the given int32 and assigns it to the NumberOfElements field.
func (o *PageResponsePonPortInformationDTO) SetNumberOfElements(v int32) {
	o.NumberOfElements = &v
}

// GetSize returns the Size field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetSize() int32 {
	if o == nil || IsNil(o.Size) {
		var ret int32
		return ret
	}
	return *o.Size
}

// GetSizeOk returns a tuple with the Size field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.Size) {
		return nil, false
	}
	return o.Size, true
}

// HasSize returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasSize() bool {
	if o != nil && !IsNil(o.Size) {
		return true
	}

	return false
}

// SetSize gets a reference to the given int32 and assigns it to the Size field.
func (o *PageResponsePonPortInformationDTO) SetSize(v int32) {
	o.Size = &v
}

// GetTotalElements returns the TotalElements field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetTotalElements() int64 {
	if o == nil || IsNil(o.TotalElements) {
		var ret int64
		return ret
	}
	return *o.TotalElements
}

// GetTotalElementsOk returns a tuple with the TotalElements field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetTotalElementsOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalElements) {
		return nil, false
	}
	return o.TotalElements, true
}

// HasTotalElements returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasTotalElements() bool {
	if o != nil && !IsNil(o.TotalElements) {
		return true
	}

	return false
}

// SetTotalElements gets a reference to the given int64 and assigns it to the TotalElements field.
func (o *PageResponsePonPortInformationDTO) SetTotalElements(v int64) {
	o.TotalElements = &v
}

// GetTotalPages returns the TotalPages field value if set, zero value otherwise.
func (o *PageResponsePonPortInformationDTO) GetTotalPages() int32 {
	if o == nil || IsNil(o.TotalPages) {
		var ret int32
		return ret
	}
	return *o.TotalPages
}

// GetTotalPagesOk returns a tuple with the TotalPages field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PageResponsePonPortInformationDTO) GetTotalPagesOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalPages) {
		return nil, false
	}
	return o.TotalPages, true
}

// HasTotalPages returns a boolean if a field has been set.
func (o *PageResponsePonPortInformationDTO) HasTotalPages() bool {
	if o != nil && !IsNil(o.TotalPages) {
		return true
	}

	return false
}

// SetTotalPages gets a reference to the given int32 and assigns it to the TotalPages field.
func (o *PageResponsePonPortInformationDTO) SetTotalPages(v int32) {
	o.TotalPages = &v
}

func (o PageResponsePonPortInformationDTO) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PageResponsePonPortInformationDTO) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Content) {
		toSerialize["content"] = o.Content
	}
	if !IsNil(o.First) {
		toSerialize["first"] = o.First
	}
	if !IsNil(o.Last) {
		toSerialize["last"] = o.Last
	}
	if !IsNil(o.Number) {
		toSerialize["number"] = o.Number
	}
	if !IsNil(o.NumberOfElements) {
		toSerialize["numberOfElements"] = o.NumberOfElements
	}
	if !IsNil(o.Size) {
		toSerialize["size"] = o.Size
	}
	if !IsNil(o.TotalElements) {
		toSerialize["totalElements"] = o.TotalElements
	}
	if !IsNil(o.TotalPages) {
		toSerialize["totalPages"] = o.TotalPages
	}
	return toSerialize, nil
}

type NullablePageResponsePonPortInformationDTO struct {
	value *PageResponsePonPortInformationDTO
	isSet bool
}

func (v NullablePageResponsePonPortInformationDTO) Get() *PageResponsePonPortInformationDTO {
	return v.value
}

func (v *NullablePageResponsePonPortInformationDTO) Set(val *PageResponsePonPortInformationDTO) {
	v.value = val
	v.isSet = true
}

func (v NullablePageResponsePonPortInformationDTO) IsSet() bool {
	return v.isSet
}

func (v *NullablePageResponsePonPortInformationDTO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePageResponsePonPortInformationDTO(val *PageResponsePonPortInformationDTO) *NullablePageResponsePonPortInformationDTO {
	return &NullablePageResponsePonPortInformationDTO{value: val, isSet: true}
}

func (v NullablePageResponsePonPortInformationDTO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePageResponsePonPortInformationDTO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


