/*
Omada Open API

Omada Open API

API version: v0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package omada

import (
	"encoding/json"
)

// checks if the AnomalySettingGridVOAnomalyEventSettingOpenApiVO type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AnomalySettingGridVOAnomalyEventSettingOpenApiVO{}

// AnomalySettingGridVOAnomalyEventSettingOpenApiVO struct for AnomalySettingGridVOAnomalyEventSettingOpenApiVO
type AnomalySettingGridVOAnomalyEventSettingOpenApiVO struct {
	// Current page number.
	CurrentPage *int32 `json:"currentPage,omitempty"`
	// Number of entries per page.
	CurrentSize *int32 `json:"currentSize,omitempty"`
	Data []AnomalyEventSettingOpenApiVO `json:"data,omitempty"`
	// The anomaly event setting creation resource, such as: 0: new created, 1: from template, 2: override
	Resource *int32 `json:"resource,omitempty"`
	// Total rows of all items.
	TotalRows *int64 `json:"totalRows,omitempty"`
}

// NewAnomalySettingGridVOAnomalyEventSettingOpenApiVO instantiates a new AnomalySettingGridVOAnomalyEventSettingOpenApiVO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAnomalySettingGridVOAnomalyEventSettingOpenApiVO() *AnomalySettingGridVOAnomalyEventSettingOpenApiVO {
	this := AnomalySettingGridVOAnomalyEventSettingOpenApiVO{}
	return &this
}

// NewAnomalySettingGridVOAnomalyEventSettingOpenApiVOWithDefaults instantiates a new AnomalySettingGridVOAnomalyEventSettingOpenApiVO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAnomalySettingGridVOAnomalyEventSettingOpenApiVOWithDefaults() *AnomalySettingGridVOAnomalyEventSettingOpenApiVO {
	this := AnomalySettingGridVOAnomalyEventSettingOpenApiVO{}
	return &this
}

// GetCurrentPage returns the CurrentPage field value if set, zero value otherwise.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetCurrentPage() int32 {
	if o == nil || IsNil(o.CurrentPage) {
		var ret int32
		return ret
	}
	return *o.CurrentPage
}

// GetCurrentPageOk returns a tuple with the CurrentPage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetCurrentPageOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentPage) {
		return nil, false
	}
	return o.CurrentPage, true
}

// HasCurrentPage returns a boolean if a field has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) HasCurrentPage() bool {
	if o != nil && !IsNil(o.CurrentPage) {
		return true
	}

	return false
}

// SetCurrentPage gets a reference to the given int32 and assigns it to the CurrentPage field.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) SetCurrentPage(v int32) {
	o.CurrentPage = &v
}

// GetCurrentSize returns the CurrentSize field value if set, zero value otherwise.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetCurrentSize() int32 {
	if o == nil || IsNil(o.CurrentSize) {
		var ret int32
		return ret
	}
	return *o.CurrentSize
}

// GetCurrentSizeOk returns a tuple with the CurrentSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetCurrentSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.CurrentSize) {
		return nil, false
	}
	return o.CurrentSize, true
}

// HasCurrentSize returns a boolean if a field has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) HasCurrentSize() bool {
	if o != nil && !IsNil(o.CurrentSize) {
		return true
	}

	return false
}

// SetCurrentSize gets a reference to the given int32 and assigns it to the CurrentSize field.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) SetCurrentSize(v int32) {
	o.CurrentSize = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetData() []AnomalyEventSettingOpenApiVO {
	if o == nil || IsNil(o.Data) {
		var ret []AnomalyEventSettingOpenApiVO
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetDataOk() ([]AnomalyEventSettingOpenApiVO, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []AnomalyEventSettingOpenApiVO and assigns it to the Data field.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) SetData(v []AnomalyEventSettingOpenApiVO) {
	o.Data = v
}

// GetResource returns the Resource field value if set, zero value otherwise.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetResource() int32 {
	if o == nil || IsNil(o.Resource) {
		var ret int32
		return ret
	}
	return *o.Resource
}

// GetResourceOk returns a tuple with the Resource field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetResourceOk() (*int32, bool) {
	if o == nil || IsNil(o.Resource) {
		return nil, false
	}
	return o.Resource, true
}

// HasResource returns a boolean if a field has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) HasResource() bool {
	if o != nil && !IsNil(o.Resource) {
		return true
	}

	return false
}

// SetResource gets a reference to the given int32 and assigns it to the Resource field.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) SetResource(v int32) {
	o.Resource = &v
}

// GetTotalRows returns the TotalRows field value if set, zero value otherwise.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetTotalRows() int64 {
	if o == nil || IsNil(o.TotalRows) {
		var ret int64
		return ret
	}
	return *o.TotalRows
}

// GetTotalRowsOk returns a tuple with the TotalRows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) GetTotalRowsOk() (*int64, bool) {
	if o == nil || IsNil(o.TotalRows) {
		return nil, false
	}
	return o.TotalRows, true
}

// HasTotalRows returns a boolean if a field has been set.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) HasTotalRows() bool {
	if o != nil && !IsNil(o.TotalRows) {
		return true
	}

	return false
}

// SetTotalRows gets a reference to the given int64 and assigns it to the TotalRows field.
func (o *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) SetTotalRows(v int64) {
	o.TotalRows = &v
}

func (o AnomalySettingGridVOAnomalyEventSettingOpenApiVO) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AnomalySettingGridVOAnomalyEventSettingOpenApiVO) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CurrentPage) {
		toSerialize["currentPage"] = o.CurrentPage
	}
	if !IsNil(o.CurrentSize) {
		toSerialize["currentSize"] = o.CurrentSize
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.Resource) {
		toSerialize["resource"] = o.Resource
	}
	if !IsNil(o.TotalRows) {
		toSerialize["totalRows"] = o.TotalRows
	}
	return toSerialize, nil
}

type NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO struct {
	value *AnomalySettingGridVOAnomalyEventSettingOpenApiVO
	isSet bool
}

func (v NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) Get() *AnomalySettingGridVOAnomalyEventSettingOpenApiVO {
	return v.value
}

func (v *NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) Set(val *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) {
	v.value = val
	v.isSet = true
}

func (v NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) IsSet() bool {
	return v.isSet
}

func (v *NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO(val *AnomalySettingGridVOAnomalyEventSettingOpenApiVO) *NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO {
	return &NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO{value: val, isSet: true}
}

func (v NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAnomalySettingGridVOAnomalyEventSettingOpenApiVO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


